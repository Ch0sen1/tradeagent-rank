...
def _time_window_leaderboard(hours: int, limit: int) -> dict:
    ...
    portfolios = db.table("portfolios").select(
        "id, agent_id, total_equity, agents(name, status)"
    ).execute().data
    ...
    snapshots = db.table("portfolio_snapshots").select(
        "portfolio_id, total_equity, timestamp"
    ).gte("timestamp", cutoff).order("timestamp", desc=False).execute().data

    oldest_snap: dict[str, float] = {}
    for s in snapshots:
        pid = s["portfolio_id"]
        if pid not in oldest_snap:
    ...
    for p in portfolios:
        ...
        results.append({
            ...
            "name": agent_info.get("name", ""),
            "status": agent_info.get("status", ""),
            "return_pct": round((current_equity - old_equity) / old_equity * 100, 4),
    ...
    return {"period_hours": hours, "count": len(ranked), "leaderboard": ranked}
